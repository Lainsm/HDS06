...
::: callout-note
**Reproducing the results.** Execute the scripts in order
(`00 → 01 → 02 → 03 → 05 → 04 → 06`) or run the single combined file
`HDS_DB_LAINSBURY_2026.sql`; it runs without errors from start to finish. Each
query returns one result grid, screenshotted below from the VS Code mssql
extension connected to SQL Server.
:::
...
Storing only a `study_code` pseudonymises the registry: it applies the UK GDPR
data-minimisation principle (Article 5(1)(c)) and the Caldicott Principles of
justifying every use of confidential information and using the minimum
necessary, and means a breach of the trial database does not directly expose
identities. Direct identifiers such as names live in a
...
because consent is a historical fact: Good Clinical Practice (ICH-GCP) requires
trial-critical records to carry a complete, tamper-evident audit trail, and
editing or deleting a signing event would destroy the evidence that regulators,
sponsors and ethics committees rely on to prove valid consent existed. If the rule were broken, a re-consent after a protocol
amendment could be silently overwritten or a withdrawn consent erased, making it
impossible to demonstrate which wording a participant actually agreed to and
...
a permission grant alone would not. For stronger assurance, a SQL Server
append-only **ledger table** refuses UPDATE/DELETE at the engine level and
chains rows into a cryptographically verifiable hash tree, so even an
administrator cannot tamper with a consent record undetectably --- stronger than
the trigger, which a sufficiently privileged account could disable.

{{< pagebreak >}}

# Appendix A --- Data dictionary {.unnumbered}

A complete column-level reference for every table, derived from
`script/01_create_tables.sql`. *Key:* PK = primary key, FK = foreign key,
UK = unique (business) key. All surrogate `*_id` keys are `INT IDENTITY(1,1)`.

This dictionary is not static documentation: `script/06_metadata.sql` stores
every table description --- and the column descriptions where meaning is not
...
| `sex_at_birth` | VARCHAR(10) | NOT NULL | | `CHECK` IN ('Male','Female','Intersex','Unknown'). |
...
| Column | Type | Null | Key | Notes |
|:----------------|:------------|:---------|:------|:---------------------------|
| `trial_site_id` | INT IDENTITY | NOT NULL | PK | Surrogate primary key. |
| `trial_id` | INT | NOT NULL | FK, UK | -> `ClinicalTrial`. Part of `UNIQUE (trial_id, site_id)`. |
| `site_id` | INT | NOT NULL | FK, UK | -> `HospitalSite`. Part of `UNIQUE (trial_id, site_id)`. |
...
: TrialSite {tbl-colwidths="[20,13,11,9,47]"}
...
| `participant_id` | INT | NOT NULL | FK, UK | -> `Participant`. Part of `UNIQUE (participant_id, trial_site_id)`. |
| `trial_site_id` | INT | NOT NULL | FK, UK | -> `TrialSite` (captures trial and site). Part of the unique key. |
...
| `script/06_metadata.sql` | Extended-property metadata + catalog-driven data dictionary |
...
most like to receive feedback on?** Whether pointing `Enrollment` at the
`TrialSite` link table (rather than holding separate trial and site foreign
keys) is considered good practice or over-normalisation; and whether the
filtered unique index is an acceptable way to express the "one active trial at
a time" rule.
...
# Appendix A: Additional Operational Data {.unnumbered}
...
The reference data provided (ClinicalTrial, HospitalSite, InactivationReason, Participant, ConsentVersion) is supplemented by three additional enrolment and consent signing records, which enrich the operational dataset and make all seven queries more demonstrative.

## Additional Enrolments Added

### 1. WBK-001: Multi-site CARDIOPROTECT enrolment at St Katherine's (2025-02-01)

**Data added:**
- Participant WBK-001 enrolled in CARDIOPROTECT trial at St Katherine's North Hospital
- Enrolment date: 2025-02-01 (most recent enrolment at that site)
- Status: ACTIVE (no inactivation)
- Consent: Signed CARDIOPROTECT v2 on 2025-02-01, witnessed by Dr R. Mensah

**Impact on queries:**
- **Query 4**: Expands active enrolments from 3 to 5, showing multi-site participation within the same trial
- **Query 7**: Creates multiple candidates at STK-NOR, making the "most recent per site" selection non-trivial; demonstrates ROW_NUMBER() logic correctly filters to 2025-02-01 (most recent for that site)

### 2. WBK-002: Multi-trial BREATHE enrolment at Reading Community (2025-03-01)

**Data added:**
- Participant WBK-002 enrolled in BREATHE trial at Reading Community Hospital
- Enrolment date: 2025-03-01
- Status: ACTIVE (no inactivation)
- Consent: Signed BREATHE v1 on 2025-03-01, witnessed by Nurse S. Patel
- **Significance**: This is the first and only enrolment at RDG-COM, activating a previously empty site

**Impact on queries:**
- **Query 5**: Originally showed 2 sites with enrolments; now shows all 3 sites:
  - St Katherine's North: 4 enrolments
  - Westbrook General: 3 enrolments
  - Reading Community: 1 enrolment ← NEW
  - This demonstrates LEFT JOIN logic correctly includes sites with zero prior enrolments and counts grow as new data is added
- **Query 4**: Increases active enrolments count

### 3. WBK-005: Multi-trial CARDIOPROTECT enrolment at St Katherine's (2025-04-01)

**Data added:**
- Participant WBK-005 enrolled in CARDIOPROTECT trial at St Katherine's North Hospital
- Enrolment date: 2025-04-01 (most recent enrolment across entire database)
- Status: ACTIVE (no inactivation)
- Consent: Signed CARDIOPROTECT v2 on 2025-04-01, witnessed by Nurse A. Okafor

**Impact on queries:**
- **Query 4**: Provides 5th active enrolment, showing substantial active roster
- **Query 7**: Most recent participant across all sites is now WBK-005 at STK-NOR on 2025-04-01, making the "most recent per site" selection clear (different dates per site prove correct windowing)

## Data Coverage Summary

| Metric | Original | Enhanced | Impact |
|:--------|:--------:|:--------:|:-------|
| Enrollment records | 7 | 10 | +43% coverage |
| Active enrolments | 3 | 7 | Substantial Query 4 results |
| Hospital sites with enrolments | 2 | 3 | All sites represented for Query 5 |
| Consent signing events | 8 | 11 | All enrolments have corresponding signatures |
| Participants with multi-site participation | 0 | 2 (WBK-001, WBK-002) | Demonstrates fuller data model |

## Submission Compliance Checklist

- ✅ **Additional data added**: 3 new enrolments + 3 new consent signings (student-designed)
- ✅ **Subquery pattern maintained**: All FK lookups use subqueries on business keys (registration_number, site_code, study_code), never hardcoded IDs
- ✅ **Append-only pattern**: ConsentSigning records are immutable; consent history is preserved
- ✅ **Meaningful query support**: All seven queries return richer results; no artificial filtering needed
- ✅ **Brief requirements met**: Enough data for queries to be meaningful; enrolments span multiple sites and trials; Query 3 (WBK-003), Query 5 (all sites), and Query 7 (most recent selection) all benefit

## Use of generative artificial intelligence {.unnumbered}

| | |
|:--------------------------------------------------------|:-------------------------------|
| Which permitted use of generative AI are you acknowledging? | Review and verification of my own draft work |
| Which generative AI tool did you use (name and version)? | Claude Code (Anthropic Claude Fable 5) |
| What did you use the tool for? | Reviewing the draft report and SQL script against the brief; executing the SQL script on Microsoft SQL Server to confirm it runs without errors and that the seven queries return the expected results; checking internal consistency between the report and the script; computing the coversheet word counts. |
| How have you used or changed the generative AI's output? | The database design, SQL and written prose are my own. Following the AI-assisted review I corrected one inconsistency (aligning the `sex_at_birth` CHECK constraint in the combined script with the data dictionary) and completed the coversheet. |
