...
extension connected to SQL Server. The *expected result* listed under each
query states the output the script reproduces.
:::
...
- ✅ **Additional data added**: 2 trial transitions (2 `UPDATE` + 2 `INSERT` on `Enrollment`, 2 `INSERT` on `ConsentSigning`), on top of the 7 enrolments / 8 signings from the worked-example minimum
- ✅ **One-active-enrolment-per-participant rule respected**: each transition closes the existing active row before opening the new one, satisfying `UX_Enrollment_OneActivePerParticipant`
- ✅ **Subquery pattern maintained**: all FK lookups (including the `UPDATE` `WHERE` clauses) use subqueries on business keys (`registration_number`, `site_code`, `study_code`), never hardcoded IDs
- ✅ **Append-only pattern**: `ConsentSigning` records are immutable; consent history is preserved
- ✅ **Original enrolment rows preserved**: closing an enrolment updates `inactivation_date`/`inactivation_reason_id` in place; no row is ever deleted
- ✅ **Meaningful query support**: Queries 4, 5 and 7 all return richer, non-trivial results without artificial filtering
