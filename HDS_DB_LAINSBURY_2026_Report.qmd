...
I have not included every column in the table below, only those where the NULLability decision is not self-evident or requires justification. The table below explains why a column is allowed to be NULL or must be NOT NULL.
...
| `ClinicalTrial.start_date`, `status`, `phase` | NOT NULL | A trial cannot exist without these columns populated. |
| `TrialSite.closed_date` | NULL | The site can still be open for recruitment on that trial. |
| `Enrollment.inactivation_date`, `inactivation_reason_id` | NULL (paired) | This is  a "meaningful absence". The paired-null `CHECK` keeps them in step. |
