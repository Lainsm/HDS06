...
My database schema (dbo.) has eight tables: the five reference tables provided as .csv files, plus three of my own. The table `TrialSite` resolves the trial-site many-to-many, `Enrolment` records a participant's participation in a trial at a site and `ConsentSigning` is the append-only audit log of consent events. The ERD is shown in crow's-foot notation in @fig-erd.
...
Every foreign key targets the surrogate key, never the business key. This renders the inserts slightly more verbose but is a personal choice for data integrity and maintainability. This study uses externally issued codes such as the ISRCTN number. If they were to change or input wrongfully, they can then be corrected in one parent row without cascading. The unique keys are mainly used in INSERT look-up subqueries. In the real world, the business keys would be known to the data-entry staff and the look-ups would be hidden behind a user interface/form or via a staging table.

### Table Descriptions

| Table | Purpose and key design choices |
|:------|:-------------------------------------------------------------------------------|
| **ClinicalTrial** | One row per trial. PK is `trial_id` (IDENTITY). UNIQUE `registration_number` (ISRCTN number) is the externally recognised natural key. Children tables reference `trial_id`, never the ISRCTN number. |
| **HospitalSite** | One row per hospital site. the PK is `site_id`. the UNIQUE `site_code` (e.g. `WBK-GEN`) are the operational natural key. Children tables reference `site_id`. |
| **Participant** | One unique pseudonymised participant per row, **no names held**. the PK is `participant_id` (IDENTITY). |
| **InactivationReason** | Controlled vocabulary of reasons a participant leaves a trial, so exits report consistently across trials. PK `reason_id` (IDENTITY). The UNIQUE `reason_code` is the stable report token.  |
| **TrialSite** | **Link table** resolving the trial-site many-to-many. It also records when each site opened/closed for recruitment on that trial. It uses a composite PK `(trial_id, site_id)`. The pairing is the row's identity, so no surrogate is used (unlike every other table). FKs to ClinicalTrial and HospitalSite. Both FKs reference the parents' surrogate keys. |
| **ConsentVersion** | Every approved version of a trial's consent form, including wording and effective date. Any old consent versions is never overwritten (under the principle of always add rather than overwrite). the PK is `consent_version_id`. there can only be one of the same version number per trial:`UNIQUE (trial_id, version_number)`. The FK references `trial_id`, not the ISRCTN number to follow the same design pattern as other tables. |
| **Enrolment** | A participant enrolled in a trial at a specific site. It carries `(trial_id, site_id)` as a composite FK straight to `TrialSite`, so trial and site are captured together and a participant can only enrol where the trial actually runs. The alternative would of been a surrogate `trial_site_id` pointer that would enforce the same rule but force every query to hop through `TrialSite`. By storing the pair keeps enrolment rows self-describing without denormalising (the composite FK keeps the pair consistent). Exit is recorded without deleting the row. PK is `enrolment_id`. The `UNIQUE (participant_id, trial_id, site_id, enrolment_date)` is a duplicate-entry guard to follow the brief: the same enrolment cannot be keyed twice on the same day (the likeliest failure when replacing spreadsheet data entry), while a participant who left a trial can still legitimately re-enrol later. Concurrent enrolment is prevented separately by the filtered index.  |
| **ConsentSigning** | Append-only log of every consent signing event. It holds when, which version, who witnessed. The PK is `consent_signing_id`. FKs are to Enrolment and ConsentVersion. `UNIQUE (enrolment_id, consent_version_id, signed_date)` treats a same-day duplicate row as a keying error while keeping a genuine re-signing of the same version on a later date (a damaged paper form re-executed) recordable. The brief requires every signing event to be recorded. The table is protected by an append-only trigger. Both FKs reference surrogate keys. the `CHECK` requires `witnessed_by` to contain at least a forename and surname for tracking purposes. |
...
clean categorical data for reporting. **Caveat:** `sex_at_birth` is interpreted as biological sex at birth (not gender identity) and captures only two values to keep the schema simple. In a production DB this would need to be reviewed against best practice and local healthcare inclusivity guidance.
...
* The paired-null check on `Enrolment` guarantees a leaving date never appears without a reason, and vice-versa.

* A `CHECK` on `witnessed_by` requires at least two words (forename + surname), so a single word or an initial alone cannot be recorded as a witness. The characters themselves are deliberately unrestricted so that names such as José or O'Brien are stored exactly as written. This matters for an auditable consent record.
...
* A filtered unique index (UX_Enrolment_OneActivePerParticipant) on Enrolment.participant_id WHERE inactivation_date IS NULL enforces the business rule that a participant may be actively enrolled in only one trial at a time as requested in the brief. This allows unlimited historical enrolments while preventing concurrent participation.
...
The two tables below cover most column in the schema. IDENTITY primary keys and `TrialSite`'s composite-PK columns are NOT NULL by definition and are omitted from below tables.
...
| Enrolment | `inactivation_date`, `inactivation_reason_id` | This is a "meaningful absence": the participant is still active in the trial. The pair must be NULL (or populated) together therefore the paired-null `CHECK` keeps them in step. |
...
| TrialSite; Enrolment; ConsentVersion | `opened_date`; `enrolment_date`; `effective_from` | Every record's timeline must begin somewhere. Unlike their end-date above, an absent value would carry no meaning. |
...
| Enrolment; ConsentSigning; ConsentVersion | **FK** columns: `participant_id`, `trial_id`, `site_id`; `enrolment_id`, `consent_version_id`; `trial_id` | A child row cannot exist without its parents. Only exception is with `inactivation_reason_id`, the one deliberately nullable FK, paired with inactivation_date. |
...
| Participant -> Enrolment | 1:M (FK `participant_id`) | A participant may have several enrolments over time, only one active at once as defined per brief (filtered index). |
| TrialSite -> Enrolment | 1:M (composite FK `trial_id, site_id`) | Each enrolment happens at exactly one trial-site pairing that the `TrialSite` link table confirms actually exists. |
| InactivationReason -> Enrolment | 1:M optional (FK `inactivation_reason_id`) | A standardised reason classifies many exits; active enrolments reference none. |
| Enrolment -> ConsentSigning | 1:M (FK `enrolment_id`) | One enrolment can have several signing events (original + re-consent after amendment). |
...
Every INSERT that references another table (e.g. Enrolment needs a trial_id) looks the ID up by the human-recognisable unique key. There is no use of hard-coded IDENTITY integers anywhere. It made the script more verbose but is best practice for maintainability, as discussed in the key design choices above. The added data is fictional and does not represent any real trial, site or participant.
...
The six standardised reasons are from the reference data (data/InactivationReason.csv). They form the controlled vocabulary that `Enrolment` references when a participant leaves a trial.
...
This data is entirely my own, the reference data contains no information about which trial runs at which site. I linked CARDIOPROTECT and BREATHE to two sites each and MOBILISE to one. Both FKs are looked up by unique key (registration number and site code).
...
This data is entirely my own. I created eight enrolments covering active, completed, withdrawn and deceased participants. They are spread across two sites (leaving one site with 0 enrolments). All three FKs are resolved by subquery from the study code, registration number and site code.
...
enrolment always has a NULL inactivation reason and an INNER join would silently drop it. Additionally, in my design, `Enrolment` stores `trial_id` directly (as part of its composite FK to `TrialSite`). Therefore `ClinicalTrial` is joined straight off it with no `TrialSite` hop needed. 
...
**Discussion.** "Active" is derived from `inactivation_date IS NULL`. Again, as I used a composite FK in my table design, `Enrolment` stores both `trial_id` and `site_id` directly, so `ClinicalTrial` and `HospitalSite`
...
```sql
SELECT hs.site_name, hs.city, COUNT(e.enrolment_id) AS total_enrolments
...
with no enrolments, and `COUNT(e.enrolment_id)` counts rows rather than NULLs so empty sites correctly score 0. `Enrolment` stores `site_id` directly, so it is
...
Storing only a `study_code` pseudonymises the registry, following the data-minimisation principle of GDPR Article 5(1)(c)[@art5gdpr]. The Caldicott Principles [@caldicott] request that every use of confidential information is justified and that the minimum necessary should be used. This reinforces Article 5(1)(c). A breach of the trial database would therefore not directly expose identities, and names add no information the registry needs to fulfil its research purpose.

Direct identifiers such as names would live in a separate, access-controlled master linkage list held by the research office. In this design, researchers can query and report entirely on `study_code`, so trial analyses can be shared more freely because they carry no personal identifiers. The cost is potential friction and the dependency on that linkage. A broken mapping could mean a participant cannot be reached. Using a pseudonymised design trades day-to-day convenience for a much stronger confidentiality and information-governance position. This is critical when dealing with personal data.
...
`script/HDS_DB_LAINSBURY_2026.sql` stores every table description  **in the database
...
## Enrolment {.unnumbered}

| Column | Type | Null | Key | Notes |
|:-------------------------|:-----------|:---------|:------|:-----------------------|
| `enrolment_id` | INT IDENTITY | NOT NULL | PK | Surrogate primary key. |
| `participant_id` | INT | NOT NULL | FK, UK | -> `Participant`. Part of `UNIQUE (participant_id, trial_id, site_id, enrolment_date)`. |
...
| `enrolment_date` | DATE | NOT NULL | UK | Date enrolment began. Part of the unique key (same-day duplicate-entry guard). |
...
: Enrolment. A filtered unique index allows only one active enrolment per participant (`WHERE inactivation_date IS NULL`). {tbl-colwidths="[27,12,10,8,43]"}
...
| `enrolment_id` | INT | NOT NULL | FK, UK | -> `Enrolment`. Part of `UNIQUE (enrolment_id, consent_version_id, signed_date)`. |
| `consent_version_id` | INT | NOT NULL | FK, UK | -> `ConsentVersion`. Part of the unique key. |
| `signed_date` | DATE | NOT NULL | UK | Date the consent was signed. Part of the unique key (same-day duplicate-entry guard). |
